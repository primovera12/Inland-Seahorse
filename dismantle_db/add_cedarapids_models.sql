-- Add Cedar Rapids make and comprehensive model list
-- Run this script ONCE in your Supabase SQL Editor

-- First, insert the make (if it doesn't exist)
INSERT INTO makes (name)
SELECT 'Cedar Rapids'
WHERE NOT EXISTS (SELECT 1 FROM makes WHERE LOWER(name) = 'cedar rapids' OR LOWER(name) = 'cedarrapids');

-- Get the make_id for Cedar Rapids and add models
DO $$
DECLARE
    cedar_rapids_id UUID;
BEGIN
    SELECT id INTO cedar_rapids_id FROM makes WHERE LOWER(name) LIKE '%cedar%rapids%' LIMIT 1;

    IF cedar_rapids_id IS NULL THEN
        INSERT INTO makes (name) VALUES ('Cedar Rapids') RETURNING id INTO cedar_rapids_id;
    END IF;

    -- ==========================================
    -- JAW CRUSHERS (Portable & Static)
    -- ==========================================
    ...
    -- ==========================================
    -- CONE CRUSHERS - RC Series (Rollercone)
    -- ==========================================
    ...
    RAISE NOTICE 'Cedar Rapids models added successfully!';
END $$;

-- Verify the insertion - count models
SELECT 'Total Cedar Rapids models:' as info, COUNT(*) as count
FROM models mo
JOIN makes m ON mo.make_id = m.id
WHERE LOWER(m.name) LIKE '%cedar%rapids%';

-- List all models
SELECT m.name as make, mo.name as model
FROM makes m
