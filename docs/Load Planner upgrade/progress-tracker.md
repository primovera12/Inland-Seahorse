...
### Current Focus
> **Phase 2 in progress.** P2-01 through P2-04 complete (4/5).
>
> **Next up:** Phase 2, Issue P2-05 — Distance defaults to 0 silently (no per-mile fees)
> **Phase file:** `phase-2-cost-accuracy.md` → Section P2-05
> **Why next:** `permit-calculator.ts` line 37 has `distanceInState: number = 0` default. When callers omit distance, all per-mile and ton-mile fees calculate as $0, silently underestimating permit costs with no warning. Should warn when distance is missing for states with distance-based fees and use a minimum fallback.
...
| 2 | Cost Accuracy | 5 | 4 | 1 | In Progress |
